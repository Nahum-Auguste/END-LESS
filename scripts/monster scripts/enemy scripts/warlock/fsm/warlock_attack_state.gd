...
var orb_burst_attack_orb_spawn_interval :float = .2
...
func enter():
	...
	setup_timer(attack_timer,3,true)
...
func get_orb_burst_attack_current_orb_direction():
	...
	return orb_burst_attack_initial_direction + Vector2(cos(deg_to_rad(a)),sin(deg_to_rad(a))).normalized() * sign(orb_burst_attack_initial_direction.length())
